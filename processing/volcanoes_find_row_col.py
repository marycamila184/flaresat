...
def main():
    df = pd.read_csv("/home/marycamila/flaresat/source/volcanoes/scenes_points_volcanoes.csv")
    ...
    for image_sat in list_images:
        ...
        path_image = os.path.join(PATH_RAW, image_sat)
        tiff_b1 = glob.glob(os.path.join(path_image, '*_B1.TIF'))[0]
        
        for entity in df_entity.itertuples():
            row, col, _ = get_row_col(entity.vulcan_longitude, entity.vulcan_latitude, tiff_b1)
    ...
    df.to_csv("/home/marycamila/flaresat/source/volcanoes/scenes_points_volcanoes.csv", index=False)

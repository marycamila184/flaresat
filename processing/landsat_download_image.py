...
def download_image(list_scenes):
    ...
    for i in range(0, len(list_scenes), WORKERS):
        ...
        if diff_auth.total_seconds() / 60 >= 115:  # Re-authenticate after 1 hour and 45 minutes
            auth_login_time, token = auth.return_token()

        headers = {'X-Auth-Token': token}
        scenes_to_download = list_scenes[i:i + WORKERS]  # Get 4 scenes
...
def main():
    ...
    print(f"Total images for {year}-{month}: {total_unique_images}")
    ...
    if remaining_images_count == 0:
        print(f"All images for {year}-{month} have been downloaded.")
        return

    print(f"{year} - Month: {month} - Remaining images for download: {remaining_images_count}")
